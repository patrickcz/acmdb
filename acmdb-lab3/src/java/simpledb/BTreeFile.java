...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
	//find the left-most if f is null
	if(f==null){
	    if(pid.pgcateg() == BTreePageId.LEAF){
	        return (BTreeLeafPage)getPage(tid, dirtypages, pid, perm);
	    }
	    BTreeInternalPage page = (BTreeInternalPage)getPage(tid, dirtypages, pid, perm);
	    BTreePageId leftId = page.iterator().next().getLeftChild();
	    return findLeafPage(tid, dirtypages, leftId , perm, f);
	}
	...
	BTreeLeafPage oldPage = null;
	if(oldPageId != null) 
	 oldPage = (BTreeLeafPage)getPage(tid, dirtypages, oldPageId, Permissions.READ_ONLY);
		
	if(oldPage != null){
		oldPage.setLeftSiblingId(newPage.getId());
		newPage.setRightSiblingId(oldPageId);	
		dirtypages.put(oldPageId, oldPage);
	}
	...
	if(field.compare(Op.GREATER_THAN_OR_EQ, mid)) return newPage;
	...
	if(field.compare(Op.GREATER_THAN_OR_EQ, mid.getKey())) return newPage;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// process the last node in parentEntry and leftSibling and update pointer of childrens
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// count the number of moved entries
		...
		// process parentEntry and the first entry of right sibling
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeInternalPage leftPage, BTreeInternalPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
		...
		// some code goes here
        // copy keys in parentEntry and set pointer
